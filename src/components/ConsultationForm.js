...
export default function ConsultationForm({ preselect = "" }) {
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label className="block text-sm font-bold text-navy mb-1.5">{t("form_name")} <span className="text-gold">*</span></label>
          ...
          <div>
            <label className="block text-sm font-bold text-navy mb-1.5">{t("form_phone")} <span className="text-gold">*</span></label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label className="block text-sm font-bold text-navy mb-1.5">{t("form_email")} <span className="text-gold">*</span></label>
            <input type="email" name="email" value={formData.email} onChange={handleChange} required
              placeholder={t("form_email_placeholder")}
              className="w-full px-4 py-3 bg-beige border border-glass-border rounded-lg text-navy placeholder-text-muted focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none transition-all text-sm" />
          </div>
          <div>
            <label className="block text-sm font-bold text-navy mb-1.5">{t("form_company")}</label>
        ...
        <div>
          <label className="block text-sm font-bold text-navy mb-1.5">{t("form_service")} <span className="text-gold">*</span></label>
  ...
  {serviceOptions.map((o) => (
    <option key={o.value} value={o.value}>{t(o.labelKey)}</option>
        ...
        <div>
          <label className="block text-sm font-bold text-navy mb-1.5">{t("form_message")}</label>
        ...
        <button type="submit" disabled={status === "loading"}
          ...
          {status === "loading" ? (
            <><svg className="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"/></svg> {t("form_submitting")}</>
          ) : (
            <>{t("form_submit")} <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg></>
          )}
        </button>
      </form>
